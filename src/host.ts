...
const allowedHostTypes = ['github', 'gitlab', 'bitbucket', 'selfhosted'] as const
...
const refTypeMapByProvider: Record<HostType, Record<Reference['type'], string>> = {
...
export function formatReference (ref: Reference, config?: HostConfig) {
  ...
  return `[${ref.value}](${baseUrl(config)}/${refTypeMap[ref.type]}/${ref.value.replace(/^#/, '')})`
}

export function formatCompareChanges (v: string, config: ChangelogConfig) {
  const part = config.host.type === 'bitbucket' ? 'branches/compare' : 'compare'
  return `[compare changes](${baseUrl(config.host)}/${part}/${config.from}...${v || config.to})`
}
...
export function getHostConfig (repository?:string): HostConfig | undefined {
  const repoUrl = repository || ''
  if (repoUrl) {
    // https://regex101.com/r/JZq179/1
    const { hostType, repo } = repoUrl.match(/(?:^(?<hostType>\w+):)?(?<repo>[\w]+\/[\w]+)(.git)?$/)?.groups ?? {}

    // https://regex101.com/r/Bdwsxu/1
    const [domain] = repoUrl.match(/(((?!-))[a-z0-9\-_]{0,61}[a-z0-9]{1,1}\.)*([a-z0-9-]{1,61}|[a-z0-9-]{1,30})\.[a-z]{2,}/) || []

    if (!repo) { return undefined }

    // there is a hostType, but it's not a valid one and we dont have a domain
    const validHostType = hostType !== 'selfhosted' && allowedHostTypes.find(type => type === hostType)
    if ((hostType && !validHostType) && !domain) { return undefined }

    const hostTypeByDomain = domain ? Object.keys(typeDomainMap).find(i => typeDomainMap[i] === domain) || 'self-hosted' : undefined
